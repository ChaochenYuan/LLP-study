...
namespace gen {
  ...
  Py8PtAndLxyGun::Py8PtAndLxyGun(edm::ParameterSet const& ps) : Py8GunBase(ps) {
    ...
    fLzMin = pgun_params.getParameter<double>("LzMin");
  ...
  bool Py8PtAndLxyGun::generatePartonsAndHadronize() {
    ...
    for (size_t i = 0; i < fPartIDs.size(); i++) {
      ...
      double vz = 0;
      double lxy = 0;
      //double VphiMax=M_PI/2;
      //double VphiMin=-M_PI/2;
      ...
      while (!passLoop) {
        ...
        for (int i = 0; i < 10000; i++) {
          double down=-1;
          double up=1;
          double vphi=asin((up-down)*randomEngine().flat()+down);
          //double vphi=(VphiMax-VphiMin)*randomEngine().flat()+VphiMin;
        ...
        for (int j = 0; j < 100; j++) {
          ...
          vz = (fLzMax-fLzMin) * randomEngine().flat() + fLzMin;  // this is abs(vz)

          float v0 = vz - fDistanceToAPEX;
          if (v0 <= 0 || lxy * lxy / (coneTheta * coneTheta) > v0 * v0 || v0 > 0) {
            passLz = true;
            break;
          }
        }
        ...
        std::cout << "pt=" << pt << ", eta" << eta << ", phi" << phi <<", px="<<px<<", py="<<py<<", pz="<<pz<< ", lxy" << lxy << ", vx=" << vx << ", vy" << vy << ", vz" << vz << ", dxy" << dxy <<", dz"<<dz<< std::endl;
        if (passLoop)
          break;
      }
